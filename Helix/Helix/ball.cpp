	...
	if (life == true)
	{
		if (speed < 0.25)
		{
			if (y < floor + 0.2 && tile_state == NORMAL_TILE)
			{
				...
				camera_follow = false;
				Squeeze = 2;
				PlaySound(L"Sound/bounce.wav", NULL, SND_ASYNC);
			}
			else if (y < floor + 0.2 && tile_state == ROTATE_TILE)
			{
				...
				Squeeze = 2;
				tower.Rotate_half();
				PlaySound(L"Sound/rotate.wav", NULL, SND_ASYNC);
			}
			...
			else if (y < floor + 0.2 && tile_state == BLIND_TILE)
			{
				...
				camera_follow = false;
				Squeeze = 2;
				PlaySound(L"Sound/spit.wav", NULL, SND_ASYNC);
		...
		if (speed >= 0.25)
		{
			...
			else if (y < floor + 0.2&&tile_state != EMPTY_TILE)
			{
				speed = -0.1;
				camera_follow = true;
				PlaySound(L"Sound/break.wav", NULL, SND_ASYNC);
				return true;
			}
		}
	}
...
void Ball::Fail()
{
	PlaySound(L"Sound/fail.wav", NULL, SND_ASYNC);
	Game_state = 4;
}

void Ball::Victory()
{
	PlaySound(L"Sound/win.wav", NULL, SND_ASYNC);
	Game_state = 5;
}
